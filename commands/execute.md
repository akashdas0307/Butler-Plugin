...
Read the local files: `TASKS.md`, `NOTIFICATIONS.md`, and `SCHEDULE.md`.

## 2. Execute Butler Tasks
Look specifically under `## Butler Tasks` in `TASKS.md`.
For each task assigned to you:
1. **Analyze**: What tools are required? 
2. **Execute**: Perform the task autonomously. 
3. **Update UI**: Move the completed task from `## Butler Tasks` to `## Done` in `TASKS.md`. Ensure you write the file to the local disk immediately so the Master's HTML Dashboard updates in real-time.
